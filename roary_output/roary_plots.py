...
if __name__ == "__main__":
    ...
    tree_tips = set(x.name for x in t.get_terminals())
    matrix_cols = set(roary_sorted.columns)
    ...
    with sns.axes_style('whitegrid'):
    	...
    	# Force the tree to stay within its subplot bounds
    	ax.set_xlim(right=mdist + mdist * 0.1)
    	ax.set_ylim(ax.get_ylim())  # lock y
    	ax.axis('off')
